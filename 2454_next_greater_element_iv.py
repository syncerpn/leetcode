# find the next greater first
# then try to encode it into a new format for find the next greater (second one)
# basically the same algorithm as the first
class Solution:
...
# two stack solution is also beautiful and clean
class Solution:
    def secondGreaterElement(self, nums: List[int]) -> List[int]:
            res = [-1] * len(nums)
