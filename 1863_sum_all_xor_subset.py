# "easy"-level backtracking problem
# we gradually consider (recursively) subset with and without i-index element in the array
class Solution:
    def subsetXORSum(self, nums: List[int]) -> int:
        def backtrack(nums, i, x):
            if i == len(nums):
                return x
            x_included = backtrack(nums, i+1, x^nums[i])
            x_excluded = backtrack(nums, i+1, x)
            return x_included + x_excluded
        
        return backtrack(nums, 0, 0)

# the bit-mask solution is just next-level shit
# i copied the explanation as well, for reference
class Solution:
    def subsetXORSum(self, nums: List[int]) -> int:
        """
        ...
        # or make it all bit manip solution with shift
